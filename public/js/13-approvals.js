    ...
    return`<div class="fade">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:12px">
        ${team.map(u=>{
          ...
          const _sc=_statCell(asgn,'assigned','#F4F9FA','#10262E','assigned')+_statCell(late,'late',late?'#FEEEEF':'#F4F9FA',late?'#C41E32':'#10262E','late')+_statCell(tkCount,'tickets',tkCount?'#F6F1E1':'#F4F9FA',tkCount?'#0B6660':'#10262E','tickets');
          return`<button onclick="S.tvUser='${u.id}';S.tvCalDate=todayISO();S.tvCalWk=0;S.tvExpanded=null;rr()"
            ...
            <div style="font-size:12px;font-weight:600;color:#0F766E;display:flex;align-items:center;gap:4px">${ic('chevR','w-3.5 h-3.5')}View submissions</div>
          </button>`;
        }).join('')}
      </div>
    </div>`;
  ...
  return`<div class="fade">
        ...
        ${week.map(d=>{
          ...
          return`<button onclick="S.tvCalDate='${d}';S.tvExpanded=null;rr();App._lazyLoadDate('teamview')" class="cal4-d ${isSel?'sel':''}" style="flex:1;min-width:0;display:flex;flex-direction:column;align-items:center;padding:10px 4px 8px;cursor:pointer;border:none;background:${isSel?'#10262E':'transparent'};gap:2px">
            ...
            <div style="display:flex;gap:2px;height:6px">
              ${hasSub?`<span style="width:5px;height:5px;border-radius:50%;background:${isSel?'rgba(255,255,255,.8)':'#D4A72C'}"></span>`:'' }
    ...
    <div style="display:flex;flex-direction:column;gap:10px;margin-top:8px">
      ${dayCls.length?dayCls.map(c=>{
        ...
        const BC={'Late':'#F2495B','Pending Approval':'#0F766E','On Time':'#D4A72C','Submitted':'#D4A72C','Pending':'#E0A106','Rejected':'#A0182A'};
