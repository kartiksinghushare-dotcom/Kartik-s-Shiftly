...
function _scopeDocsTab(type, scopeKey){
  ...
  if(isAdm||p.edit)html+='<button onclick="App._newFolderIn(\''+type+'\',\''+scopeKey+'\')" style="display:inline-flex;align-items:center;gap:6px;padding:8px 14px;border-radius:10px;border:1.5px solid #E7F0F2;background:#fff;font-size:13px;font-weight:600;cursor:pointer">📁 New folder</button>';
  if(isAdm||p.upload)html+='<button onclick="App._uploadDocIn(\''+type+'\',\''+scopeKey+'\')" style="display:inline-flex;align-items:center;gap:6px;padding:8px 14px;border-radius:10px;background:#10262E;color:#fff;font-size:13px;font-weight:600;border:none;cursor:pointer">⬆ Upload file</button>';
  html+='</div>';
  // Empty state
  ...
  if(scopeFolders.length){
    html+='<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:10px;margin-bottom:14px">'
      +scopeFolders.map(f=>'<div onclick="App._docNav(this.dataset.id)" data-id="'+f.id+'" class="fld-card" style="background:#fff;border-radius:14px;border:1.5px solid #E7F0F2;padding:14px;cursor:pointer;transition:all .15s">'
        +'<div style="font-size:28px;margin-bottom:8px">📁</div>'
        +'<div style="font-size:13px;font-weight:700">'+esc(f.name)+'</div>'
        +'<div style="font-size:11px;color:#90A5AB;margin-top:4px">'+((DB.documents||[]).filter(d=>d.folderId===f.id).length)+' files</div>'
  ...
  // Documents list
  if(scopeDocs.length){
    html+='<div style="background:#fff;border-radius:16px;border:1px solid #E7F0F2;overflow:hidden">'
      +scopeDocs.map((d,i)=>{
        const ext=(d.name.split('.').pop()||'').toLowerCase();
        const icon=ext==='pdf'?'📄':ext.match(/xlsx?|csv/)?'📊':ext.match(/docx?/)?'📝':ext.match(/pptx?/)?'📊':ext.match(/jpe?g|png|gif|webp/)?'🖼':'📎';
        return'<div style="display:flex;align-items:center;gap:12px;padding:12px 16px;border-bottom:'+(i<scopeDocs.length-1?'1px solid #F1F7F8':'none')+'">'
...
App._newFolderIn=(t,s)=>{
  ...
  openModal(
    ...
    +'<button onclick="App.closeModal()" style="background:none;border:none;cursor:pointer;color:#90A5AB;font-size:20px">×</button>'
    +'</div>'
    +'<div style="margin-bottom:16px">'
    +'<label for="nf-name" style="display:block;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:#5E767D;margin-bottom:6px">Folder name</label>'
    +'<input id="nf-name" type="text" placeholder="e.g. Procurement, Inventory…" '
    +'style="width:100%;background:#fff;border:1.5px solid #E7F0F2;border-radius:12px;padding:10px 14px;font-size:14px;outline:none;box-sizing:border-box;font-family:inherit" '
    +'autofocus/>'
    +'</div>'
    +'<div style="display:flex;gap:8px">'
    +'<button onclick="App.closeModal()" style="flex:1;padding:11px;border-radius:10px;border:1.5px solid #E7F0F2;background:#fff;font-weight:600;font-size:14px;cursor:pointer">Cancel</button>'
    +'<button onclick="App._createFolderIn()" style="flex:1;padding:11px;border-radius:10px;background:#10262E;color:#fff;font-weight:700;font-size:14px;border:none;cursor:pointer">Create folder</button>'
    +'</div>'
    +'</div>',
    'max-w-sm'
  );
...
function _subDeptsTabHTML(d){
  ...
  h+='<div style="font-size:13px;color:#5E767D">'+subs.length+' sub-department'+(subs.length===1?'':'s')+'</div>';
  if(can('subDepartments','create'))h+='<button onclick="App.editDept(null,\''+d.id+'\')" style="display:inline-flex;align-items:center;gap:6px;padding:8px 14px;border-radius:10px;background:#10262E;color:#fff;font-size:13px;font-weight:700;border:none;cursor:pointer">'+ic('plus','w-4 h-4')+'Add sub-department</button>';
  h+='</div>';
...
function deptsPage(){
  ...
  if(sel){
    ...
    return'<div class="fade">'
      ...
      +'</div>'
      // Sub-tabs
      +'<div style="display:flex;gap:4px;margin-bottom:16px;background:#F4F9FA;border-radius:12px;padding:4px">'
      +TABS.map(([k,l])=>'<button onclick="App._setDeptTab(this.dataset.k)" data-k="'+k+'" style="flex:1;padding:8px;border-radius:9px;font-size:13px;font-weight:700;border:none;cursor:pointer;background:'+(stab===k?'#fff':'transparent')+';color:'+(stab===k?'#10262E':'#5E767D')+';box-shadow:'+(stab===k?'0 1px 4px rgba(0,0,0,.08)':'none')+'">'+l+'</button>').join('')
      +'</div>'
      // Tab content
      ...
      +(stab==='users'
        ?('<div class="space-y-2">'
          +(dUsers.length
            ?dUsers.map(u=>'<div style="display:flex;align-items:center;gap:12px;background:#fff;border-radius:14px;border:1px solid #E7F0F2;padding:12px 14px">'+avatar(u,'w-10 h-10','text-xs')+'<div style="flex:1"><div style="font-size:14px;font-weight:600">'+esc(fullName(u))+'</div><div style="font-size:12px;color:#90A5AB">'+esc(u.position||u.role)+'</div></div>'+chip(u.status)+'</div>').join('')
      ...
      +(stab==='checklists'
        ?('<div class="space-y-2">'
          +(dCls.length
            ?dCls.map(c=>'<div style="display:flex;align-items:center;gap:12px;background:#fff;border-radius:14px;border:1px solid #E7F0F2;padding:12px 14px"><div style="width:36px;height:36px;border-radius:10px;background:#F4F9FA;display:grid;place-items:center;flex-shrink:0">'+ic('list','w-4 h-4')+'</div><div style="flex:1"><div style="font-size:14px;font-weight:600">'+esc(c.name)+'</div><div style="font-size:12px;color:#90A5AB">'+esc(c.frequency)+' · '+(c.assignees||[]).length+' assigned</div></div></div>').join('')
...
function locsPage(){
  ...
  if(sel){
    ...
    return'<div class="fade">'
      ...
      +((can('locations','edit')||can('locations','delete'))?(can('locations','edit')?'<button onclick="App.editLoc(this.dataset.id)" data-id="'+l.id+'" style="display:inline-flex;align-items:center;gap:6px;padding:7px 14px;border-radius:10px;background:#F4F9FA;color:#2F4C55;font-size:13px;font-weight:600;border:1px solid #E7F0F2;cursor:pointer">'+ic('edit','w-4 h-4')+'Edit</button>':'')+(can('locations','delete')?'<button onclick="App.delLoc(this.dataset.id)" data-id="'+l.id+'" style="display:inline-flex;align-items:center;gap:6px;padding:7px 14px;border-radius:10px;background:#FEEEEF;color:#C41E32;font-size:13px;font-weight:600;border:1px solid #FBCDCD;cursor:pointer">'+ic('trash','w-4 h-4')+'Delete</button>':''):'')
      +'</div>'
      +'<div style="display:flex;gap:4px;margin-bottom:16px;background:#F4F9FA;border-radius:12px;padding:4px">'
      +TABS.map(([k,ll])=>'<button onclick="App._setLocTab(this.dataset.k)" data-k="'+k+'" style="flex:1;padding:8px;border-radius:9px;font-size:13px;font-weight:700;border:none;cursor:pointer;background:'+(stab===k?'#fff':'transparent')+';color:'+(stab===k?'#10262E':'#5E767D')+';box-shadow:'+(stab===k?'0 1px 4px rgba(0,0,0,.08)':'none')+'">'+ll+'</button>').join('')
      +'</div>'
      +(stab==='docs'?_scopeDocsTab('loc',l.id):'')
      +(stab==='checklists'
        ?('<div class="space-y-2">'
          +(lCls.length
            ?lCls.map(c=>'<div style="display:flex;align-items:center;gap:12px;background:#fff;border-radius:14px;border:1px solid #E7F0F2;padding:12px 14px"><div style="width:36px;height:36px;border-radius:10px;background:#F4F9FA;display:grid;place-items:center;flex-shrink:0">'+ic('list','w-4 h-4')+'</div><div style="flex:1"><div style="font-size:14px;font-weight:600">'+esc(c.name)+'</div><div style="font-size:12px;color:#90A5AB">'+esc(c.frequency)+'</div></div></div>').join('')
      ...
      +(stab==='info'
        ?'<div class="bg-white rounded-2xl border border-ink-100 p-5 space-y-3">'
          +[['Name',l.name],['Address',l.address||'—'],['Department',l.department||'All departments'],['Status',l.status||'Active']].map(([k,v])=>'<div><div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:#90A5AB;margin-bottom:2px">'+k+'</div><div style="font-size:14px;font-weight:600">'+esc(v)+'</div></div>').join('')
          +'</div>'
        :'')
      +'</div>';
  }
